...
exports.getAddRecord = async (req, res) => {
  try {
    return res.render("record/add-record", {
      ...
      error: null,
      member: await Member.find({ membershipStatus: "active" }).sort({
        uId: 1,
      }),
...
exports.postEditRecord = async (req, res) => {
  try {
    ...
    const member = await Member.find({ membershipStatus: "active" }).sort({
      name: 1,
    });
    const availableBooks = await Book.find({ status: "available" }).sort({
      title: 1,
    });
    const bookQuantity = selectedBook.availableQuantity;
    const borrowedQuantity = selectedBook.borrowedQuantity;
...
exports.deleteRecord = async (req, res) => {
  try {
    ...
    return res.render("record/record-details", {
      ...
      error: err,
      edit: false,
      record: await Record.find().populate("book.bookId").populate("memberId"),
      oldInput: {},
    });
  }
};
...
exports.filterRecord = async (req, res) => {
  try {
    const { name, email, book, issueDate, dueDate, status } = req.body;
    ...
    if (status) {
      filter.push({
        "book.status": { $regex: new RegExp(`${status}`, "i") },
      });
    }
    ...
    res.render("record/record-details", {
      title: "Record Details Page",
      record: findedRecord,
      error: null,
      oldInput: req.body,
...
exports.filterByIssueDate = async (req, res) => {
  try {
    const { issue, filter } = req.body;

    let recordFinded = [];
    if (filter == "up") {
      recordFinded = await Record.find({
        "book.issueDate": { $gte: issue },
      })
        .populate("memberId")
        .populate("book.bookId");
    } else if (filter == "down") {
      recordFinded = await Record.find({
        "book.issueDate": { $lte: issue },
      })
        .populate("memberId")
        .populate("book.bookId");
    }
    ...
    return res.render("record/record-details", {
      title: "Record Details Page",
      record: await Record.find().populate("memberId").populate("book.bookId"),
      error: error,
      oldInput: {},
    });
  }
...
exports.filterByDueDate = async (req, res) => {
  try {
    const { due, filter } = req.body;

    let recordFinded = [];
    if (filter == "up") {
      recordFinded = await Record.find({
        "book.dueDate": { $gte: due },
      })
        .populate("memberId")
        .populate("book.bookId");
    } else if (filter == "down") {
      recordFinded = await Record.find({
        "book.dueDate": { $lte: due },
      })
        .populate("memberId")
        .populate("book.bookId");
    }
